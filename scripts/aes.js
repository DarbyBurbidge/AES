...
export const KeyExpansionCore = (input, idx) => {
...
export const KeyExpansion = (inputKey, expandedKeys) => {
    for(let i = 0; i < 16; i++) {
    ...
    while(bytesGenerated < 176) {
        ...
        for(let i = 0; i < 4; i++) {
            expandedKeys[bytesGenerated] = expandedKeys[bytesGenerated - 16] ^ tempCore[i];
            bytesGenerated++;
        }
    }
}
...
export const UnMixColumns = (state) => {
    let tmp = []
...
export const AddRoundKey = (state, roundKey, roundNum) => {
    for(let i = 0; i < 16; i++) {
        state[i] ^= roundKey[(16 * roundNum) + i];
    }
}

export const RemoveRoundKey = (state, roundKey, roundNum) => {
    for(let i = 15; i >= 0; i--) {
        state[i] ^= roundKey[(16* roundNum) + i];
    }
}

export const AES_Decrypt = (message, initialKey) => {
    let state = []
    for (let i = 0; i < 16; i++) {
        state[i] = message[i];
    }
    ...
    //reversing the key is easier than rewriting the AddRoundKey fn
    ...
    for(let i = 0; i < state.length;i++) {
        if(state[i]) {
            finalMessage[i] = String.fromCharCode(parseInt(state[i]));
        }
    }
    return finalMessage;
}

export const AES_Encrypt = (message, initialKey) => {
    let state = []
    for (let i = 0; i < 16; i++) {
        state[i] = message[i];
    }
    ...
    let finalMessage = [];
    for(let i = 0; i < 16; i++) {
        finalMessage[i] = addLeadingZeroes(state[i].toString(16))

    }
    return finalMessage
}
export const addLeadingZeroes = (message) => {
    if(message.length >= 2) {
        return message
    }
    return `0${message}`
...
export const runAES = (message, decrypt) => {
    const key = [
        0x01, 0x02, 0x03, 0x04,
        0x05, 0x06, 0x07, 0x08,
        0x09, 0x0A, 0x0B, 0x0C,
    ...
    let lengthOfPaddedMessage = originalLength
    if(lengthOfPaddedMessage % 16 != 0) {
        lengthOfPaddedMessage = Math.floor(lengthOfPaddedMessage / 16 + 1) * 16;
    }
    let paddedMessage = []
    if (decrypt) {
        for(let i = 0; i < message.length;i++){
            if (!Number.isNaN(message[i])) {
                paddedMessage[i] = message[i]
            }
        }
        lengthOfPaddedMessage = paddedMessage.length
    } else {
        for(let i = 0; i < lengthOfPaddedMessage; i++) {
            if (i >= originalLength) {
                paddedMessage[i] = 0;
            } else {
                paddedMessage[i] = Number(message.charCodeAt(i));
            }
        }
    }
    ...
    for(let i = 0; i < lengthOfPaddedMessage; i += 16) {
        const tempMessage = []
        for(let k = 0; k < 16; k++) {
            tempMessage[k] = paddedMessage[i + k];
        }
        if (decrypt) {
            console.log(tempMessage)
            finishedMessage.push(AES_Decrypt(tempMessage, key));
        } else {
            finishedMessage.push(AES_Encrypt(tempMessage, key));
        }
        
    }
    if (decrypt) {
        return finishedMessage.toString().replace(/((?<!,)|(?=[,]{2})),/g, "");
    }
    return finishedMessage.toString().replace(/,/g, " ")
};
